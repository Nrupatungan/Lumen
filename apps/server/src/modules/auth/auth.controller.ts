...
import {registerSchema, loginSchema, requestPasswordResetSchema, resetPasswordSchema} from "../../lib/validators/auth.validator.js"
...
export const register: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const token = await UserService.createVerificationToken(user._id.toString());
...
export const login: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const user = await UserService.verifyCredentials(parsed.email, parsed.password);
  
  if (!user) {
    res.status(401).json({ message: "Invalid credentials or user not verified." });
    return;
  }
 ...
 */
export const requestPasswordReset: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const token = await UserService.createPasswordResetToken(user._id.toString());
